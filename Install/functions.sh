...
function sudoLinkFile()
{
    ...
    ## If the destination file is a link, remove it
    if [ -L "${2}" ]; then
        sudo /usr/bin/env rm --force --verbose "${2}"
    fi
    
    ## If the destination file exists, rename it
    if [ -e "${2}" ]; then
        sudo mv --force --verbose "${2}" "${2}".$(getDate).ori
    fi

    ## Copies the file
    sudo ln --force --verbose --logical --symbolic "${1}" "${2}"    
}
